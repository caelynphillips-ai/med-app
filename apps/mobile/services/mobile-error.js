export function describeMobileError(error, action = "Action") {
  ...
  if (code === "auth/admin-restricted-operation" || code === "auth/operation-not-allowed") {
    return "Preview mode could not start because Firebase Anonymous sign-in is not enabled. In Firebase Console, enable Authentication > Sign-in method > Anonymous.";
  }

  if (code === "permission-denied" || /missing or insufficient permissions/i.test(message)) {
    return `${action} was blocked by Firebase permissions. Make sure you are signed in with Preview mode and that Firestore rules allow this account to read and write its own user data.`;
  }

  if (code === "unauthenticated") {
    return `${action} needs an active Firebase session. Use Preview mode before saving or syncing medications.`;
  }
